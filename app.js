...
app.route("/movie/:movieid")
  .get(function(req, res) { //FIX THIS: ADD number of likes and user rating
    ...
    if (req.cookies.userData) {
      ...
      )  y RIGHT JOIN (select email as emul,imdbID as Liked, movieName as title from likeList WHERE likeList.imdbID = ?
      ) z ON z.Liked = y.imdbID WHERE email is null) a WHERE email = ? OR emul = ?
      ...
      `;
        connection.query(cQuery, [mId, mId, mId, mId, req.cookies.userData.email, req.cookies.userData.email, mId], function(error, results, fields) {
          if (error) {
            console.log(error);
        ...
        if (jsonRes.Response === "False"){
                  ...
                  res.render("movie", {
                    ...
                    imdbLink: imdbLink,
                    imdbRating: mIMDB,
                    rotHidden: rottenHide,
                    rotLink: rotLink,
                    rotRating: mRotten,
                    likeOpt: likeOption,
                    likeText: likeText,
                    rateCount: numberOfRaters,
